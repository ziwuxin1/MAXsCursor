...
| `MAXsCursor-Setup-v1.1.exe` | Installer (per-user, no admin) | ~68 MB | Standard Start-menu entry + Windows **Apps & Features** uninstall. |
...
Output: `installer\MAXsCursor-Setup-v1.1.exe`.
...
| `MAXsCursor-Setup-v1.1.exe` | 安装包（per-user，无需管理员） | 约 68 MB | 标准开始菜单入口 + Windows **应用与功能** 里可卸载 |
...
输出 `installer\MAXsCursor-Setup-v1.1.exe`。
